...
use super::{BackendError, BackendExecutionContext, BackendFromDir, BackendGraph, BackendInner, NamedTensor};
    ...
    fn compute(
        ...
        match inputs {
            ...
            Some(inputs) => {
                ...
                for input in &inputs {
                    let index = self.inputs.iter().position(|slot| slot.shape.name == input.name);
                    let index = match index {
                        Some(idx) => idx,
                        None => {
                            // Try to convert name to index (e.g., "0", "1", etc.)
                            if let Ok(idx) = input.name.parse::<usize>() {
                                ...
                                return Err(BackendError::BackendAccess(anyhow::anyhow!(
                                    "Unknown input tensor name: {}", input.name
                                )));
                            }
                        }
                    };
