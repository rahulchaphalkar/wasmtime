...
pub fn main() -> Result<(), Error> {
    ...
    let image = fs::read("./kitten.png").unwrap();
    ...
    let output = context.get_output(0, &mut output_buffer[..])?;
...
fn softmax(output_tensor: Vec<f32>) -> Vec<f32> {
    let max_val = output_tensor.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
