use crate::dsl::{align, fmt, inst, r, rex, rw, sxl, sxq, sxw};
...
pub fn list() -> Vec<Inst> {
    vec![
        ...
        // inst("comisd", fmt("A", [rw(xmm), r(xmm_m64)]), rex([0x66, 0x0F, 0x2F]).r(), _64b | compat | sse2), // Todo rw(xmm) to w(xmm)
        // inst("comiss", fmt("A", [rw(xmm), r(xmm_m32)]), rex([0x0F, 0x2F]).r(), _64b | compat | sse), // Todo rw(xmm) to w(xmm)
        inst("ucomisd", fmt("A", [r(xmm), r(xmm_m64)]), rex([0x66, 0x0F, 0x2E]).r(), _64b | compat | sse2),
        inst("ucomiss", fmt("A", [r(xmm), r(xmm_m32)]), rex([0x0F, 0x2E]).r(), _64b | compat | sse),
    ]
}