        ...
        f.add_block(
            ...
            |f| {
                f.add_block(
                    ...
                    ["cmppd", "cmpps", "cmpsd", "cmpss"].contains(&self.mnemonic.as_str()) &&
                    self.format.operands.len() == 3 &&
                    matches!(self.format.operands[2].location.kind(), dsl::OperandKind::Imm(_)) {
                        let imm_operand = self.format.operands[2].location;
                     f.add_block(&format!("let mnemonic = match self.{}.value()", imm_operand), |f| {
